...
class WorkoutDay(models.Model):
    ...
    class Meta:
        # Change from alphabetical to natural week order
        ordering = [
            Case(
                When(day_name='Monday', then=Value(1)),
                When(day_name='Tuesday', then=Value(2)),
                When(day_name='Wednesday', then=Value(3)),
                When(day_name='Thursday', then=Value(4)),
                When(day_name='Friday', then=Value(5)),
                When(day_name='Saturday', then=Value(6)),
                When(day_name='Sunday', then=Value(7)),
